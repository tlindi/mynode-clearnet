...
read_inputs() {
    ...
    if [ -f "$HTTP_HOSTS_FILE" ]; then
        log "Reading extra hostnames from $HTTP_HOSTS_FILE"
        ...
        log "No $HTTP_HOSTS_FILE file found; only using ${HTTPS_BASE_CERT} and ${HTTPS_DOMAIN}"
    fi
}
...
#
resolve_host_ips() {
    # returns newline-separated IPs for given hostname; no caching here
    local host="$1"
    local ips=()
...
check_mx_for_domain() {
    ...
    if command -v host >/dev/null 2>&1; then
        ...
        # host prints "has no MX record" when none; otherwise contains "mail is handled" lines
